...
const ExperienceItem: React.FC<{ title: string; company: string; duration: string; location: string; description: string; logo: string }> = ({ title, company, duration, location, description, logo }) => (
    <View style={tw`flex-row gap-4 mb-4`}>
        <Image source={imageAssets.avater} style={tw`w-10 h-10 rounded-lg mt-1`} />
        ...
        <ScrollView style={tw`flex-1 bg-[#0F0E13]`}>
            <View>
                <TouchableOpacity onPress={() => handleImagePick('cover')}>
                    <Image source={imageAssets.postimg} style={tw`w-full h-48`} />
                ...
                <View style={tw`absolute top-0 w-full mt-32`}>
                    <View style={tw`items-center`}>
                        <TouchableOpacity onPress={() => handleImagePick('avatar')}>
                            <Image source={imageAssets.postimg} style={tw`w-24 h-24 rounded-full border-4 border-[#3A3E41]`} />
                            <SvgXml style={tw`absolute bottom-2 right-0`} xml={editImageicon} />
                        </TouchableOpacity>

                        <TouchableOpacity onPress={() => router.push('/drawer/screens/Edit_business_profile')} style={tw`absolute -bottom-4 right-4 bg-[#4593F5] py-1.5 px-3 rounded-md flex-row items-center gap-2`}>
                            <SvgXml xml={editicon} />
            ...
            <View style={tw`p-4`}>
                ...
                <View style={tw`mt-6`}>
                    <View style={tw`flex-row justify-between items-center mb-4`}>
                        ...
                        <TouchableOpacity onPress={() => router.push('/drawer/screens/Post_A_job')} style={tw`bg-[#4593F5] py-1 px-3 rounded-md flex-row items-center gap-2`}>
